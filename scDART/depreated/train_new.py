...
def _train_mmd(model, opts, b_x_rna, b_x_atac, reg_mtx, dist_atac, dist_rna, 
    ...
    b_z_atac = model["encoder"](torch.cat((model["gene_act"](b_x_atac),torch.zeros(b_x_atac.shape[0],1).to(device)), dim = 1) )
    b_z_rna = model["encoder"](torch.cat((b_x_rna, torch.ones(b_x_rna.shape[0],1).to(device)), dim = 1) )
...
def match_latent(model, opts, dist_atac, dist_rna, data_loader_rna, data_loader_atac, 
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % 10 == 0:
            ...
            loss = ave_loss_mmd + ave_loss_d_atac + ave_loss_rna + ave_loss_genact + ave_loss_anchor
            
            if loss.item() < best_loss:
                best_loss = loss.item()
                torch.save(model, '../check_points/model.pt')
                count = 0
            else:
                count += 1
                if count % 20 == 0:
                    ...
                    print('Epoch: {}, shrink lr to {:.4f}'.format(t + 1, opt.param_groups[0]['lr']))
                    if opt.param_groups[0]['lr'] < 1e-6:
                        break
                    else:
                        model = torch.load('../check_points/model.pt', map_location = device)
                        count = 0
